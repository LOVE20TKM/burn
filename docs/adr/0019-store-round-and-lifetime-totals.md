...
Burn 按地址、社区和类别保存仅包含发生变化轮次的稀疏累计检查点，并以最新检查点作为全周期累计值，避免重复保存累计 mapping。地址单轮只保存实际数量，得分由该轮冻结系数派生；社区单轮数量和得分由本轮与前一轮累计检查点的差值派生。同轮操作更新最后一个检查点，跨轮操作复制前值后追加检查点。查询在合约内二分查找不晚于目标轮次的最后检查点，读取复杂度为 `O(log k)`，其中 `k` 是该地址或社区在该类别发生变化的轮次数；写入只访问最后一个检查点，复杂度为 `O(1)`。
